...
const BlogDetail = () => {
  ...
  useEffect(() => {
    ...
    fetch(`https://your-api-url.com/api/follows/${id}`)
  ...
  const handleFollow = () => {
    if (blog?.name) {
      // Proceed with follow logic using blog.author.id
      console.log('Author ID:', blog.name);
      // Your follow logic here, for example:
      fetch(`http://136.228.158.126:50001/api/follows/${blog.name}`, {
        method: 'POST',
        headers: {
      ...
      .catch(error => console.error('Error following author:', error));
    } else {
      console.error('Author ID is missing or undefined:', blog);
    }
  };
